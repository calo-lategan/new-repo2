#!/usr/bin/env bash
# v5 image-view chain: rqt_image_view -> image_view -> browser.
#
...
#
# Usage:
#   image_view_chain.sh [topic]              default: /custom_sortingv5/image_result
#
# Env:
...
# get applied only when JETARM_V5_QT_SAFE=1 - they cap rqt's paint
...
if [ "${JETARM_V5_QT_SAFE:-0}" = "1" ]; then
    export QT_X11_NO_MITSHM=1
    export QT_QPA_PLATFORM=xcb
    export LIBGL_ALWAYS_SOFTWARE=1
fi

TOPIC="${1:-/custom_sortingv5/image_result}"
...
# report "not responding". v5 starts web_video_server from our launch
