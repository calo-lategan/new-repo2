...
# v5.1: the app source dir. v5.1 is the finalised app, so its installer must
# deploy v5.1's OWN node/UI/launch/profiles. (This read 'custom_sortingv5' in
# the copied script - which would have symlinked v5's code instead, so v5.1
# would never actually deploy. Pointing it at custom_sortingv5.1 fixes that and
# makes the slow/medium/fast.yaml seeding below resolve to v5.1's profiles.)
V4="$SRC_DIR/custom_sortingv5.1"
if [ ! -d "$V4" ]; then
    err "v5.1 app folder not found inside the checkout: $V4"
    exit 1
fi
ok "source ready"
...
# default.yaml is the single boot source of truth (the tuner UI's per-tab
# Save & Apply + "Save ALL as default" merge into it). v5.1 also seeds the
# three speed presets (slow/medium/fast.yaml) so the Slow/Medium/Fast buttons
# work out of the box - they are PARTIAL profiles loaded on demand via
# load_profile and are never auto-loaded at boot. An existing on-device file is
# kept (so operator edits survive). yolo.yaml is NOT seeded (it used to revert
# saved model knobs from a stale sample; the node ignores it at boot now).
for p in default.yaml fast.yaml medium.yaml slow.yaml; do
    src="$V4/profiles/$p"
    dst="$PROFILES_DIR/$p"
    if [ -f "$dst" ]; then
...
Other files installed:
  ...
  $PROFILES_DIR/{default,slow,medium,fast}.yaml
