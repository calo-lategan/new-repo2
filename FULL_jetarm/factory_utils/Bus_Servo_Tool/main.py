    ...
    def button_clicked(self, name):
        if name == 'read':
            ...
            try:
                ...
                time.sleep(0.01)
                angle_limit = getServoAngleLimit(id)
                if  angle_limit is not None and angle_limit[0] != pos_min and angle_limit[1] != pos_max:
                    self.message_from('set pulse failed！')
                    return 
                setServoVinLimit(id, vin_min*100, vin_max*100)
                time.sleep(0.01)
                vin_limit = getServoVinLimit(id)
                print(vin_limit)
                if  vin_limit is not None and vin_limit[0] != vin_min * 100 and vin_limit[1] != vin_max * 100:
            ...
            try:
                ...
                if getServoID() != 1:
                    self.message_from('set id failed！')
                    return
                servo_id = int(self.lineEdit_servoID.text())
                ...
                time.sleep(0.01)
                angle_limit = getServoAngleLimit(1) 
                if angle_limit is not None and angle_limit[0] != 0 and angle_limit[1] != 1000:
                    self.message_from('set pulse failed！')
                    return          
                setServoVinLimit(1, 4500, 14000)
                time.sleep(0.01)
                vin_limit = getServoVinLimit(1) 
                if vin_limit is not None and vin_limit[0] != 4500 and vin_limit[1] != 14000:
                    self.message_from('set voltage failed！')
                    return             
