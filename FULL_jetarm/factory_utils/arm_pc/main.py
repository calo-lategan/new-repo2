...
class MainWindow(QtWidgets.QWidget, Ui_Form):
    def __init__(self):
        ...
        self.servo_index = {'6':10, '5':5, '4':4, '3':3, '2':2, '1':1}
        self.horizontalSlider_index = {'1':6, '2':5, '3':4, '4':3, '5':2, '6':1} 
        ...
        self.kinematics_run.pressed.connect(lambda: self.button_run('ik_run'))
        ...
        self.Button_replace.pressed.connect(lambda: self.button_flie_operate('replace'))
    ...
    def language(self, name):
        if self.radioButton_zn.isChecked() and name.text() == '中文':
            ...
            self.Button_replace.setText("覆盖保存")
            ...
            self.Button_replace.setText("Overwrite")
    ...
    def fk_process(self):
        data = []
        for i in range(self.joints):
            data.append(int(self.lineEdit_object[i].text()))
        pulse = transform.pulse2angle(data)  # 舵机脉宽值转为弧度
        # print('input:', pulse)
        res = self.fk.get_fk(pulse)  #获取运动学正解
        # print('output:', res)
        rpy = list(transform.qua2rpy(res[1]))
        # print('rpy:', rpy)
        if abs(res[0][0]) < 1e-8:
            res[0][0] = 0
        if abs(res[0][1]) < 1e-8:
        ...
        if abs(rpy[1]) < 1e-8:
            rpy[1] = 0
        self.lineEdit_x.setText(str(res[0][0]))
        self.lineEdit_y.setText(str(res[0][1]))
        self.lineEdit_z.setText(str(res[0][2]))
        self.lineEdit_pitch.setText(str(rpy[1]))
        self.lineEdit_x.setCursorPosition(0)
        self.lineEdit_y.setCursorPosition(0)
        self.lineEdit_z.setCursorPosition(0)
        self.lineEdit_pitch.setCursorPosition(0)
        self.lineEdit_pitch_min.setText('-180')
        self.lineEdit_pitch_max.setText('180')
    ...
    def button_editaction_clicked(self, name):
        ...
        if name == 'servoPowerDown':
            for servo_id in range(1, self.joints + 1):
                unloadServo(self.servo_index[str(servo_id)])
            if self.chinese:
    ...
    def button_run(self, name):
        if name == 'run':
            ...
            # print(x, y, z, pitch, pitch_min, pitch_max)
            all_solutions = get_ik([x, y, z], pitch, [pitch_min, pitch_max])
            if all_solutions != []:
                current_servo_positions = []
                for i in range(self.joints - 2):
                    current_servo_positions.append(int(self.lineEdit_object[i].text()))
                rpy = []
                min_d = 1000*4
                optimal_solution = []
                for s in all_solutions:
                    pulse_solutions = transform.angle2pulse(s[0])
                    # print(pulse_solutions)
                    for i in pulse_solutions:
                        ...
                        if min_sum < min_d:
                            ...
                            rpy = s[1]
                            optimal_solution = i
                # print(optimal_solution, rpy)
                self.lineEdit_pitch.setText(str(round(rpy[1], 5)))
                self.lineEdit_pitch_min.setText('-180')
                self.lineEdit_pitch_max.setText('180')
                for i in range(self.joints - 2):
                    servo_pulse = int(round(optimal_solution[i]))
                    self.horizontalSlider_pulse_object[i].setValue(servo_pulse)
                    self.lineEdit_object[i].setText(str(servo_pulse))
                    setServoPulse(self.servo_index[str(i + 1)], servo_pulse, 500)
            else:
                self.message_from('no solution!')
            self.ik_control = False
            self.resetServos_ = False
            ...
            if name == 'saveActionGroup':
                ...
                if path != '':                    
                    ...
                    if path[-4:] == '.d6a':
                        ...
                        self.action_group_name.setText(str(path) + '.d6a')
            if name == 'replace':
                path = self.action_group_name.text()
                if os.path.isfile(path):
                    os.system('sudo rm ' + path)
                if path != '':                    
                    if path[-4:] == '.d6a':
                    ...
                    self.reflash_action()
                    if self.chinese:
                        result = self.message_delect('保存成功!')
                    else:
                        result = self.message_delect('success')  
