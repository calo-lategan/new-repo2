...
class YoLov8TRT(object):
    ...
    def infer_from_camera(self):
        cap = cv2.VideoCapture(0)  # 替换为摄像头路径或编号
        if not cap.isOpened():
        ...
        while True:
            ...
            # 推理
            self.ctx.push()  # 激活TensorRT的context
        ...
        # Remove any context from the top of the context stack, deactivating it.
            self.ctx.pop()  # 弹出context
        # Here we use the first row of output in that batch_size = 1
            output = self.host_outputs[0]

            # 后处理推理结果
            result_boxes, result_scores, result_classid = self.post_process(
                output[:self.det_output_length], origin_h, origin_w, rotated=True
            )

            # 绘制结果框并显示
            for i in range(len(result_boxes)):
                box = result_boxes[i]
                np.random.seed(int(result_classid[i]))
                color = [np.random.randint(0, 255) for _ in range(3)]
                plot_one_box(
                    box,
                    image_raw,
                    label="{}:{:.2f}".format(categories[int(result_classid[i])], result_scores[i]),
                    color=color,
                    line_thickness=1,
                    rotated=True
                )
            ...
            # 在图像上绘制FPS信息
            cv2.putText(image_raw, f"FPS: {fps:.2f}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)            

            # 显示处理后的图像
            cv2.imshow("YOLOv8 OBB - Detection", image_raw)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 开始从摄像头推理
        yolov8_wrapper.infer_from_camera()
