...
static std::vector<cv::Point> xywhr2xyxyxyxy(const Detection& box) {

    float cos_value = std::cos(box.angle);
    float sin_value = std::sin(box.angle);
    float x1 = box.bbox[0];
    float y1 = box.bbox[1];
    float w = box.bbox[2] / 2, h = box.bbox[3] / 2;
    float vec1_x = w * cos_value, vec1_y = w * sin_value;
    float vec2_x = -h * sin_value, vec2_y = h * cos_value;

    std::vector<cv::Point> corner_points;
    corner_points.push_back(cv::Point(x1 + vec1_x + vec2_x, y1 + vec1_y + vec2_y));
    corner_points.push_back(cv::Point(x1 + vec1_x - vec2_x, y1 + vec1_y - vec2_y));
    corner_points.push_back(cv::Point(x1 - vec1_x - vec2_x, y1 - vec1_y - vec2_y));
    corner_points.push_back(cv::Point(x1 - vec1_x + vec2_x, y1 - vec1_y + vec2_y));
...
void draw_bbox_obb(std::vector<cv::Mat>& img_batch, std::vector<std::vector<Detection>>& res_batch) {
    ...
    for (size_t i = 0; i < img_batch.size(); i++) {
        ...
        for (auto& obj : res) {
            ...
            auto corner_points = xywhr2xyxyxyxy(obj);
