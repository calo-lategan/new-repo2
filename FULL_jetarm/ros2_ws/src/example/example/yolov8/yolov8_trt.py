#!/usr/bin/python3
#coding=utf8
# YOLOv8 OBB identifies nodes YOLOv8 OBB识别节点
"""
...
class YoLov8TRT(object):
    ...
    def infer(self, raw_image_generator, conf):
        ...
        for i in range(self.batch_size):
            ...
            result_boxes, result_scores, result_classid = self.post_process(output[i * self.det_output_length: (i + 1) * self.det_output_length], batch_origin_h[i],batch_origin_w[i], conf, rotated=True)
    ...
    def post_process(self, output, origin_h, origin_w, conf, rotated=False):
        """
        ...
        boxes = self.non_max_suppression(pred, origin_h, origin_w,
                                         conf_thres=conf, nms_thres=IOU_THRESHOLD, rotated=True)
        if rotated:
    ...
    if test:
        ...
        while running:
            ...
            if frame is not None:
                ...
                boxes, scores, classid  = yolov8_wrapper.infer(frame, conf)
