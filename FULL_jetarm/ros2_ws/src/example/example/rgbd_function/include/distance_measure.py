...
# Implement Distance Measurement实现距离测量
# By default, display the distance of the point closest to the camera. 默认情况下回显示距离相机最近的点的距离
# You can measure the distance of any point on the screen by clicking it with the mouse. 可以鼠标点击画面任意一点测量对应点的距离
# You can use the mouse to revert to measuring the distance of the closest point. 可以鼠标恢复最近点测量
...
class DistanceMeasureNode(Node):
    ...
    def __init__(self, name):
        ...
        # Synchronize the timestamp. The time is allowed to have an error of 0.02 seconds. 同步时间戳, 时间允许有误差在0.02s
        sync = message_filters.ApproximateTimeSynchronizer([rgb_sub, depth_sub], 4, 0.5)
        sync.registerCallback(self.multi_callback) # Execute the feedback function. 执行反馈函数
