#!/usr/bin/env python3
"""
ps2 remote control, when the mode is 0, it is the single servo mode. When the mode is 1, it is the coordinate mode. ps2 手柄机体遥控， mode为0为单舵机模式， mode为1为坐标模式
"""
...
OLD_AXES_MAP = 'ly', 'lx', 'ry', 'rx'
# OLD_BUTTON_MAP = 'A', 'B', '', 'X', 'Y', '', 'l1', 'r1', 'l2', 'r2', 'select', 'start', '', 'l3', 'r3', '', 'hat_xd', 'hat_yr', 'hat_yl', '', 'laxis_r','laxis_l', 'laxis_d', 'laxis_u', 'raxis_r',  'raxis_l', 'raxis_d', 'raxis_u'
OLD_BUTTON_MAP = 'A', 'B', '', 'X', 'Y', '', 'l1', 'r1', 'l2', 'r2', 'select', 'start', '', 'l3', 'r3', 'hat_yl', 'hat_yr', 'hat_xd', 'hat_xu', 'laxis_l', 'laxis_r', 'laxis_u', 'laxis_d', 'raxis_l', 'raxis_r', 'raxis_u', 'raxis_d'

NEW_AXES_MAP = 'ly', 'lx', 'ry', 'rx', 'hat_x', 'hat_y'
NEW_BUTTON_MAP = 'Y', 'B', 'A', 'X', 'l1', 'r1', 'l2', 'r2', 'select', 'start','', '', 'mode',  'hat_yl', 'hat_yr','hat_xd', 'hat_xu'
...
class JoystickController(Node):
    def __init__(self, name):
        ...
        self.last_buttons = dict(zip(self.buttons_map, [0.0, ] * len(self.buttons_map)))
    ...
    def joy_change(self): 
        if os.path.exists("/dev/input/by-id/usb-2563_USB_WirelessGamepad-event-joystick"):
            self.axes_map = NEW_AXES_MAP
            self.buttons_map = NEW_BUTTON_MAP
        else:
            self.axes_map = OLD_AXES_MAP
            self.buttons_map = OLD_BUTTON_MAP
    ...
    def joy_callback(self):
        ...
        # Check if currently inserted. 检查当前是否插入
        self.count += 1
        if self.count > 10:
        ...
        # Joystick value 摇杆值
        axes = dict(zip(self.axes_map, axes))

        # Convert the ad value of joystick to bool value. 摇杆 ad 值转为 bool 值
        ...
        # Convert the ad value of direction cap to bool value. 方向帽的 ad 值转为 bool 值
        hat_x, hat_y = hat
        hat_xl, hat_xr = 1 if hat_x == 1 else 0, 1 if hat_x == -1 else 0
        hat_yd, hat_yu = 1 if hat_y == 1 else 0, 1 if hat_y == -1 else 0

        buttons.extend([hat_xl, hat_xr, hat_yu, hat_yd])
        buttons.extend([laxis_l, laxis_r, laxis_u, laxis_d, raxis_l, raxis_r, raxis_u, raxis_d])
        buttons = dict(zip(self.buttons_map, buttons))
        # print(buttons)

        # Determine whether the joystick value is changed. 判断摇杆值的是否改变
        axes_changed = False
        for key, value in axes.items():  # The axle value is changed. 轴的值被改变
