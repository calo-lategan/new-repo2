...
class PacketControllerState(enum.IntEnum):
    # The format of the communication protocol. 通信协议的格式
...
class PacketFunction(enum.IntEnum):
    # The control function that can be realized through the serial port. 可通过串口实现的控制功能
    PACKET_FUNC_SYS = 0
    PACKET_FUNC_LED = 1  # LED control LED控制
    PACKET_FUNC_BUZZER = 2  # Buzzer control 蜂鸣器控制
    PACKET_FUNC_MOTOR = 3  # Motor control 电机控制
    PACKET_FUNC_PWM_SERVO = 4  # PWM servo control, on the board, from the inside to the outside, they are 1-4 in sequence. PWM舵机控制, 板子上从里到外依次为1-4
    PACKET_FUNC_BUS_SERVO = 5  # Bus servo control 总线舵机控制
    PACKET_FUNC_KEY = 6  # Key acquisition 按键获取
    PACKET_FUNC_IMU = 7  # IMU acquisition IMU获取
    PACKET_FUNC_GAMEPAD = 8  # Handle acquisition 手柄获取
    PACKET_FUNC_SBUS = 9  # Remote-controlled acquisition of model aircraft 航模遥控获取
    PACKET_FUNC_OLED = 10 # OLED display content Settings OLED 显示内容设置
    PACKET_FUNC_NONE = 11

class PacketReportKeyEvents(enum.IntEnum):
    # Different states of the keys. 按键的不同状态
...
class Board:
    ...
    def __init__(self, device="/dev/ttyUSB0", baudrate=1000000, timeout=5):
        ...
        # Queues are used to store data. 队列用来存储数据
    ...
    def get_battery(self):
        # Obtain the voltage in mAh. 获取电压，单位mAh
        if self.enable_recv:
            try:
    ...
    def get_button(self):
        # Obtain the status of the keys key1 and key2, and return the key ID(1 indicates key1 and 2 indicate key2) and status (0 indicates pressed, 1 indicates released).获取按键key1， key2状态，返回按键ID(1表示按键1，2表示按键2)和状态(0表示按下，1表示松开)
        if self.enable_recv:
            try:
    ...
    def get_imu(self):
        # Obtain the IMU data and return ax, ay, az, gx, gy, gz. 获取IMU数据，返回ax, ay, az, gx, gy, gz
        if self.enable_recv:
            try:
                # ax, ay, az, gx, gy, gz
    ...
    def set_oled_text(self, line, text):
        data = [line, len(text)] # The subcommand 0x01 sets the SSID. The second byte is the string length, which includes the '\0' string terminator. 子命令为 0x01 设置 SSID, 第二个字节是字符串长度，该长度包含'\0'字符串结束符
...
if __name__ == "__main__":
    ...
    # board.set_motor_speed([[1, 100], [2, 100], [3, -0.6], [4, -0.6]])
