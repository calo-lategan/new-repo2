...
def get_hand_landmarks(img_size, landmarks):
    """
    Convert landmarks from the normalized output of mediapipe to pixel coordinates. 将landmarks从mediapipe的归一化输出转为像素坐标
    :param img: Pixel coordinates corresponding image 像素坐标对应的图片
...
def hand_angle(landmarks):
    """
    Calculate the blending angle of each finger 计算各个手指的弯曲角度
...
def h_gesture(angle_list):
    """
    Determine the gesture formed by the fingers through two-dimensional features. 通过二维特征确定手指所摆出的手势
    :param angle_list: The blending angle of each finger. 各个手指弯曲的角度
...
class HandGestureNode(Node):
    def __init__(self, name):
        ...
        self.buzzer = buzzer.BuzzerController()  # fps calculator fps计算器
        ...
        self.create_service(SetBool, '~/enable_trace', self.start_srv_callback, callback_group=self.timer_cb_group) # Start the feature 进入玩法
    ...
    def main(self):
        while self.running:
            ...
            bgr_image = cv2.flip(bgr_image, 1) # Mirrored image 镜像画面
            result_image = np.copy(bgr_image) # Make a copy for result display to prevent modification of the image during processing. 拷贝一份用作结果显示，以防处理过程中修改了图像
            ...
            if self.start:
                try:
                    results = self.hand_detector.process(bgr_image) # Hand and key points recognition 手部、关键点识别
                    if results.multi_hand_landmarks and self.draw :
                        ...
                        for hand_landmarks in results.multi_hand_landmarks: 
                            ...
                            gesture = h_gesture(angle_list) # Judge gesture based on key points position. 根据关键点位置判断手势
